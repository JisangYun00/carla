    ...
    public:
      ...
      HmcCommandSubscriber(void* vehicle, std::string /*base_topic_name*/, std::string frame_id) :
        ...
        _last_ad02_time(std::chrono::steady_clock::now()) {
          if (!_ad01_impl->Init(GetBaseTopicName() + "01")) {
            log_warning("HmcCommandSubscriber: Init failed for topic: ", GetBaseTopicName(), "01");
          }
          if (!_ad02_impl->Init(GetBaseTopicName() + "02")) {
            log_warning("HmcCommandSubscriber: Init failed for topic: ", GetBaseTopicName(), "02");
          }
        }
